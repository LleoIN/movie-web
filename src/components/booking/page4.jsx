...
export default function Page4(props) {
  ...
  function getfood(){
    return props.food.map((elm, key) => {
        return(
            <div style={{margin:"50px 70px 25px 10px",fontSize:"24px"}} className="box1" key={key}>
                <img style={{width:"200px",height:"200px"}} src={require(`../../image/food/${elm.image}`)} alt="" />
                {foodall[elm.name]>0?<button style={{fontSize:"16px",borderRadius:"50%",width:"30px",margin:"0 0 0 -30px",position:"relative",top:"120px",left:"-170px"}} onClick={(e) => {changereduce(e);}} name={elm.name} value={elm.price}>
                        -
                </button>:" "}
                <h3 style={{margin:"0 0 0 10px",width:"180px",textAlign: 'center'}}>
                     {elm.name} {elm.price}
                </h3>
                <button style={{zIndex:"1",fontSize:"16px",borderRadius:"50%",width:"30px",position:"relative",top:"-23px",left:"170px"}} onClick={(e) => {changefood(e);}} name={elm.name} value={elm.price}>
                        +
                </button>
